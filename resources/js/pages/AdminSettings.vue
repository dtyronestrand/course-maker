  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    ...
    >
      <div class="p-8">
        ...
        <form @submit.prevent>
          <input type="number" v-model="capacity" class="max-w-xs px-4 text-black" />
          <Button @click="handleSettings" class="btn btn-primary mt-4 ml-4">Save</Button>
        </form>
      </div>
      <div class="p-8">
        ...
        <div v-for="cycle in page.props.developmentCycles" :key="cycle.id" class="mb-4">
          <div class="flex flex-row flex-nowrap justify-between">
            ...
            <div class="flex space-x-2">
              <Pencil @click="editCycle(cycle)" class="text-amber-500" :size="20" />
              <Trash2 @click="deleteCycle(cycle)" class="text-red-500" :size="20" />
            </div>
          </div>
        ...
        <form v-if="addCycle" @submit.prevent class="mt-4 flex flex-col gap-4">
          <div>
      ...
      <div class="p-8">
        ...
        <form v-if="addDeliverable" @submit.prevent class="mt-4 flex flex-col gap-4">
          <div>
          ...
          >
          <Button @click="addDeliverable = false" class="btn btn-secondary mr-2"
            >Cancel</Button
          >
        </form>
      </div>
...
const cycle = ref<DevelopmentCycle>({ name: '', start_date: null, end_date: null } as DevelopmentCycle);
