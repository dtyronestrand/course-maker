<template>
  <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div v-if="edit !== team.id">
          ...
          <div class="mb-4 flex flex-col items-center">
            <UserInfo
              role="uppercase"
              avatar="lg"
              v-if="team.users.find((user: User) => user.role === 'lead')"
              :user="team.users.find((user: User) => user.role === 'lead')!"
              :user_role="'lead'"
            />
          </div>
    ...
    <Modal title="Create Team" :visible="visible" @close="visible = false">
      ...
      <template #footer>
        <button @click="createTeam" class="rounded bg-blue-500 p-2 text-white">
          Create
        </button>
        <button
          @click="visible = false"
