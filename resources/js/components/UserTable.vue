      ...
      if (isEditing.value && selectedUserId.value === row.original.id) {
        return h(
          "select",
          {
            ...
            class:
              "w-full rounded border border-amber-500 p-1 text-sm bg-slate-300 text-slate-900",
          },
      ...
      if (isEditing.value && selectedUserId.value === row.original.id) {
        return h(
          "select",
          {
            ...
            class:
              "w-full rounded border border-gray-300 p-1 text-sm bg-white text-slate-900",
          },
          [
            h("option", { value: "" }, "N/A"),
      ...
      if (isThisRowEditing) {
        return h("div", { class: "flex gap-2 items-center" }, [
          h(
            ...
            {
              class:
                "rounded bg-green-500 px-3 py-1 text-xs text-white hover:bg-green-600 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 outline-none",
              onClick: (e: Event) => {
                e.stopPropagation();
                saveEdit(user.id);
              },
          ...
          h(
            ...
            {
              class:
                "rounded bg-red-500 px-3 py-1 text-xs text-white hover:bg-red-600 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 outline-none",
              onClick: (e: Event) => {
                e.stopPropagation();
                cancelEdit();
              },
          ...
          h(
            "button",
            {
              ...
              class:
                "text-red-500 cursor-pointer hover:text-red-700 ml-1 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 outline-none rounded",
              onClick: (e: Event) => {
                e.stopPropagation();
                deleteUser(user.id);
              },
        ...
        return h(
          ...
          {
            class:
              "flex gap-3 items-center opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity",
          },
          [
            h(
              "button",
              {
                ...
                class:
                  "text-amber-500 cursor-pointer hover:text-amber-700 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 outline-none rounded",
                onClick: (e: Event) => {
                  e.stopPropagation();
                  startEdit(user);
                },
              },
              [h(Pencil, { class: "w-6 h-6" })],
            ),
          ],
        );
      }
