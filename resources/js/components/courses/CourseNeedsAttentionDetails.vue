<template>
    ...
    >
        <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="course-needs-attention-title"
            class="mx-auto max-w-3xl rounded-lg border border-primary bg-surface-container p-4 shadow-sm shadow-primary"
        >
            <div class="mb-4 flex items-center justify-between">
                <h2 id="course-needs-attention-title" class="text-2xl font-bold">
                    {{ props.course.prefix }} {{ props.course.number }}
                </h2>
                <button
                    @click="$emit('modal-close')"
                    class="rounded-md text-primary hover:text-primary-focus focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
