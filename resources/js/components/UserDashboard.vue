<template>
    ...
    <div class="mt-4">
    <ul>
    <li class="px-4" v-for="deliverable in course.deliverables" :key="deliverable.id"><label class="flex flex-row items-center justify-between cursor-pointer">{{ deliverable.name}}<input type="checkbox" v-model="deliverable.pivot.is_done" class="cursor-pointer"/></label></li>
    </ul>
    </div>
