<template>
  <div>
    ...
    <div class="grid grid-cols-4 gap-4">
      <div
        class="col-span-3 flex gap-4 flex-row flex-wrap border border-secondary! p-4"
      >
        <div
          v-for="course in props.courses"
          :key="course.id"
          class="border border-primary! w-[30%] p-4 rounded-lg glass"
        >
          <header>
          ...
          <div class="mt-4">
            <ul>
              ...
              >
                <label
                  class="flex flex-row align-items justify-between cursor-pointer w-full"
                  >{{ deliverable.name
                  }}<input type="checkbox" v-model="deliverable.pivot.is_done"
                /></label>
              </li>
            </ul>
          </div>
          <div class="mt-4 flex justify-center flex-row gap-4">
            <button
              class="bg-primary text-[var(--on-primary)] text-sm px-4 py-2 rounded-lg focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 outline-none"
            >
              Course Map
            </button>
            <button
              class="bg-secondary text-[var(--on-secondary)] text-sm px-4 py-2 rounded-lg focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 outline-none"
            >
              Course Storyboard
            </button>
          </div>
